...
class Speaking(Toplevel,):
    def __init__(self):
        ...
        self.show_button1 = tkinter.Button(self, text="例文を見せる", command=lambda:self.ShowExample())
        self.show_button1.grid(row=0, column=0)

        self.show_button2 = tkinter.Button(self, text="答えを見せる", command=lambda:self.ShowAnswer())
        self.show_button2.grid(row=0, column=1)

        self.right_button = tkinter.Button(self, text="正しい", command=lambda:self.AddPoint())
        self.right_button.grid(row=1, column=0)

        self.wrong_button = tkinter.Button(self, text="間違い", command=lambda:self.SubtractPoint())
    ...
    def GetCard(self):
        ...
        if status == "GOOD":
            ...
            tkinter.Label(self, text="言葉認識をもっと勉強して").grid(column=0, row = 0)
